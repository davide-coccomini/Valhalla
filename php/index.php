...
$row=$result->fetch_assoc();
	...
	$_SESSION["datachestspeciale"]=$row["datachestspeciale"];
   	$_SESSION["saccheggio"]=$row["Saccheggio"];
?>
	...
	<div id="content">
		...
		<div ID="menuleft">
			...
			<?php
				...
				$result=$mysqli->query($query);
				$row=$result->fetch_assoc();        
                $query="SELECT COUNT(*) as numero FROM messaggisistema WHERE Nuovo=1 AND Destinatario='".$_SESSION['username']."'";
				$result=$mysqli->query($query);
				$rowsistema=$result->fetch_assoc();
                $numero=$rowsistema["numero"]+$row["numero"];
				$_SESSION["nuovimess"]=$numero;
